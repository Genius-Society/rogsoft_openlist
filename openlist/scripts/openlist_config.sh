...
check_run_mode() {
	if [ $(check_usb2jffs_used_status) == "1" ] && [ "${1}" == "start" ]; then
		echo_date "➡️检测到已安装插件 usb2jffs 并成功挂载, 插件可以正常启动!"
		restore_openlist_used_db
	fi
}
	...
	if [ "${ACT}" = "start" ]; then
		if [ $(check_usb2jffs_used_status) != "1" ]; then #未挂载usb2jffs就检测是否需要运行在/tmp目录
			...
			if [ "$LINUX_VER" = 41 ]; then #内核过低就运行在Tmp目录
				...
				echo_date "⚠️安装 usb2jffs 插件并成功挂载可恢复正常运行模式!"
...
makeConfig() {
			...
			if [ -n "${openlist_cert_file}" -a -n "${openlist_key_file}" ]; then
				...
				if [ -f "${openlist_cert_file}" -a -f "${openlist_key_file}" ]; then
					...
					if [ -n "${CER_VERFY}" -a -n "${KEY_VERFY}" ]; then
						...
						echo_date "⚠️无法启用 https, 原因如下:"
						if [ -z "${CER_VERFY}" ]; then
							echo_date "⚠️证书公钥 Cert 文件错误, 检测到这不是公钥文件!"
							dbus set openlist_cert_error=1
						fi
						if [ -z "${KEY_VERFY}" ]; then
							echo_date "⚠️证书私钥 Key 文件错误, 检测到这不是私钥文件!"
							dbus set openlist_key_error=1
						fi
					fi
				else
					...
					if [ ! -f "${openlist_cert_file}" ]; then
						echo_date "⚠️未找到证书公钥 Cert 文件!"
						dbus set openlist_cert_error=1
					fi
					if [ ! -f "${openlist_key_file}" ]; then
						echo_date "⚠️未找到证书私钥 Key 文件!"
						dbus set openlist_key_error=1
					fi
				fi
			else
				...
				if [ -z "${openlist_cert_file}" ]; then
					echo_date "⚠️证书公钥 Cert 文件路径未配置!"
					dbus set openlist_cert_error=1
				fi
				if [ -z "${openlist_key_file}" ]; then
					echo_date "⚠️证书私钥 Key 文件路径未配置!"
					dbus set openlist_key_error=1
				fi
			fi
		fi
	...
	if [ "${configHttps}" == "true" ]; then
		if [ "${configHttpsPort}" == "${configPort}" ]; then
			configHttps=false
			configHttpsPort="-1"
			echo_date "⚠️OpenList 管理面板 http 和 https 端口相同, 本次启动关闭 https!"
		else
			if [ "${openlist_force_https}" == "1" ]; then
				echo_date "🆗 OpenList 管理面板已开启强制跳转 https。"
	...
	if [ "${openlist_publicswitch}" == "1" ]; then
		if [ -n "${openlist_site_url}" ]; then
			...
			if [ "$?" != "0" ]; then
				echo_date "⚠️网站 URL: ${openlist_site_url} 格式错误!"
				echo_date "⚠️本次插件启动不会将此网站 URL 写入配置, 下次请更正, 继续..."
				...
				if [ -n "${MATCH_2}" ]; then
					# ddnsto, 不能开https
					if [ "${configHttps}" == "true" ]; then
						echo_date "⚠️网站 URL: ${openlist_site_url} 来自 ddnsto!"
						echo_date "⚠️你需要关闭 OpenList 的 https, 不然将导致无法访问面板!"
					fi
				else
					# ddns, 根据情况判断
					if [ -n "${MATCH_3}" -a "${configHttps}" != "true" ]; then
						echo_date "⚠️网站 URL: ${openlist_site_url} 格式为 https!"
						echo_date "⚠️你需要启用 OpenList 的https功能, 不然会导致面 OpenList 部分功能出现问题!"
					elif [ -n "${MATCH_4}" -a "${configHttps}" == "true" ]; then
						echo_date "⚠️网站 URL: ${openlist_site_url} 格式为 http!"
						echo_date "⚠️你需要启用 OpenList 的https, 或者更改网站 URL 为 http 协议, 不然将导致无法访问面板!"
					else
						...
						if [ -z "${MATCH_5}" ]; then
							echo_date "⚠️网站 URL: ${openlist_site_url} 端口配置错误!"
							echo_date "⚠️你需要为网站 URL 配置端口: ${rightPort}, 不然会导致面 OpenList 部分功能出现问题!"
						fi
					fi
				fi
...
start)
	if [ "${openlist_enable}" == "1" ]; then
		...
		logger "[软件中心-开机自启]: OpenList 自启动成功!"
	else
		logger "[软件中心-开机自启]: OpenList 未开启, 不自动启动!"
	fi
	;;
boot_up)
	if [ "${openlist_enable}" == "1" ]; then
...
start_nat)
	if [ "${openlist_enable}" == "1" ]; then
		if [ "${openlist_publicswitch}" == "1" ]; then
			...
			logger "[软件中心-NAT重启]: OpenList 未开启公网访问, 不打开湍口!"
		fi
	fi
	;;
backup)
...
web_submit)
	...
	if [ "${openlist_enable}" == "1" ]; then
		...
		echo_date "ℹ️停止 openList!" | tee -a ${LOG_FILE}
